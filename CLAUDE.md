...
- **Trades on the *previous* day's predictions.** `_resolve_signal_date` resolves strictly to the prior trading day. Missing signal data produces a transient local safety plan (forced local exits, all other holdings preserved) that is applied without persistence so a repaired restart can run normal planning.
